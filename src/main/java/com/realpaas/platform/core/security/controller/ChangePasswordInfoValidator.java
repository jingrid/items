 ...
 */
public class ChangePasswordInfoValidator implements Validator {

    @Override
    public boolean supports(Class<?> clazz) {
        return ChangePasswordInfo.class.isAssignableFrom( clazz );
    }

    @Override
    public void validate(Object target, Errors errors) {
        ChangePasswordInfo vo = (ChangePasswordInfo)target;
        ...
        if( !StringUtils.hasLength( vo.getNewPassword() ) ) {
            errors.rejectValue("newPassword", "app.password.null");
        }
        else{
            if (!UserAccessHelper.checkPasswordValidity( vo.getNewPassword() )) {
                errors.rejectValue("newPassword", "app.password.illegal");
            }
        }
    }
