...
Recorder.addEventHandler('clickLocator', 'click', function(event) {
		if (event.button == 0) {
			...
			if (clickable) {
                ...
                var value = null;
                if( clickable.clickableType == "link" ){
                    value = clickable.innerHTML;
                }
                else if( clickable.clickableType == "button" ){
                    value = clickable.innerHTML;
                }
                else if( clickable.clickableType == "input" ){
                    value = clickable.value;
                }
                else{
                    value = "";
                }
                this.record("click", this.findLocators(event.target), value);
            } else {
                var target = event.target;
...
Recorder.prototype.findClickableElement = function(e) {
	...
	if (e.hasAttribute("onclick") || e.hasAttribute("href") || tagName == "button" ||
        ...
        if(tagName == "a"){
            e.clickableType = "link";
        }
        else if(tagName == "button"){
            e.clickableType = "button";
        }
        else if(tagName == "input"){
            e.clickableType = "input";
        }
        else{
            e.clickableType = "";
        }
